    ...
    <div class="transaction-details-container scroll">
      ...
      >
        ...
        <div v-if="t.mosaics" class="asset-item">
          <span>{{ $t('assets') }}:</span>
          <div
            v-for="(a, index2) in renderMosaicsAndReturnArray(t.mosaics, $store)"
            :key="index2"
            class="modify-item"
          >
            <div v-if="!a.name">
        ...
        <div v-if="t.cosignatories" class="asset-item">
          ...
          <div
            v-for="(item, index2) in t.cosignatories.modifications"
            :key="index2"
            class="modify-item"
          >
            <div>
        ...
        <span v-for="(value, key) in t" :key="key">
          <span v-if="!unusedAttributesList.find(item => item === key) && value !== ''" class="other-info">
            <span>{{ $t(key) }}:&nbsp;</span>
            <span>{{ value }}</span>
          </span>
          <span v-if="key === 'namespace'" class="other-info">
            <span>{{ $t(key) }}:&nbsp;</span>
        ...
        <div v-if="cosignedBy && cosignedBy.length" class="asset-item">
          ...
          <div v-for="(item, index3) in cosignedBy" :key="index3" class="modify-item">
            <div>
              <span class="title">{{ index3 + 1 }}:&nbsp;</span>
